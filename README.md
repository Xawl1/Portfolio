...
* 🎴 Custom project cards — five projects (an ASP.NET MVC API, two demo business sites, two Unity prototypes) each with a bespoke inline SVG icon and gradient header
* 🖱️ One-click email copy — no `mailto:` link, just a button that copies my email to the clipboard with a "Copied!" confirmation
* 🎬 Scroll-reveal animations — sections fade and slide in via `IntersectionObserver`, and everything respects `prefers-reduced-motion`
